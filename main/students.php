...
<a  href="index.php"><button class="btn btn-default btn-large" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Back</button></a>
			<?php 
			include('../connect.php');
				$result = $db->prepare("SELECT * FROM Info ORDER BY Studname DESC ");
				$result->execute();
...
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<?php
				...
				include('../connect.php');
				$result = $db->prepare("SELECT * from Info Order by Studname;");
				$result->execute();
...
<td><?php echo $row['StudID']; ?></td>
			...
			<td><?php echo $row['StudSection']; ?></td>
			<td><?php echo $row['RFID']; ?></td>
			<td><?php	
				if ($row['StudStatus'] == "1") {
  				echo "SUBMITTED";
				} elseif ($row['StudStatus'] == "0") {
  				echo "NOT SUBMITTED";

				}
			?></td>
			<td>
			<a  title="Click to edit the Student" href="editstudent.php?id=<?php echo $row['StudID']; ?>"><button class="btn btn-warning btn-mini"><i class="icon-edit"></i> Edit</button> </a>
